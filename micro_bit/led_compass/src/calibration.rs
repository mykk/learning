    ...
    I: I2c
{
    ...
    while samples < PERIMETER_POINTS {
        ...
        if leds[cursor.0][cursor.1] != 1 {
            ...
            let mag_data = measurement_to_enu(sensor.magnetic_field().unwrap());
...
pub fn calibrated_measurement(measurement: MagneticField, calibration: &Calibration) -> Measurement {
...
fn measurement_to_enu(measurement: MagneticField) -> Measurement {
    Measurement {
        x: -measurement.y_nt(),
        y: -measurement.x_nt(),
        z: measurement.z_nt(),
